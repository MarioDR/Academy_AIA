...
def get_face_bbox_from_keypoints(keypoints, img_w, img_h, padding=0.5):
    ...
    """
    # Filtra solo i primi 5 keypoints (x, y) ignorando la confidenza se presente
    face_kpts = keypoints[:5]
    
    # Rimuovi i keypoints non rilevati (es. coordinate a 0)
    valid_kpts = [kp for kp in face_kpts if kp[0] > 0 and kp[1] > 0]
    
    if len(valid_kpts) < 2:
        return None # Non ci sono abbastanza punti per definire il volto
    ...
    # Calcola larghezza e altezza del volto dai keypoints
    w = x_max - x_min
    h = y_max - y_min
    
    # Se i punti sono troppo vicini, stima una dimensione base
    if w == 0: w = 20
    if h == 0: h = 20
    ...
    # Rendiamo il ritaglio possibilmente quadrato
...
def main():
    ...
    if args.mode == "image":
        ...
        while True:
            ...
            # Mostra l'ultimo volto e corpo rilevati (per non aprire troppe finestre in loop)
            if len(face_rois) > 0:
